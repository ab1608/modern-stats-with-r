...
# Scenario: A new fertilizer is being tested to see if it helps plants grow taller.
# Group A (Control): Plants grown with standard soil.
# Group B (Treatment): Plants grown with the new fertilizer.
# The Question: Does the fertilizer actually make plants grow taller, or is any difference just due to random chance?

# Null Hypothesis (H_0): There is no difference in the mean height between the two groups (mu_A = mu_B).
# Alternative Hypothesis (H_a): There is a significant difference (mu_A != mu_B).
...
plant_data <- data.frame(
  height = c(control, treatment),
  group = rep(c("Control", "Treatment"), each = 30)
)

# 3. Two-sided t-test
results <- t.test(height ~ group, data = plant_data, alternative = "two.sided")
